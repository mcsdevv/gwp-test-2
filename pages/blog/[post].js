...
class Single extends React.Component {
	...
	render() {
		...
		return(
			<>
				<Head>
					<title key="blog-post-title">{seo_title}</title>
					<meta charset="utf-8"/>
				...
				<div className={`${singleStyles.pageContent}`}>
					<div className={`row ${singleStyles.breadcrumbRow}`}>
					...
					<div className={`row ${singleStyles.pageHeader} text-center`}>
						<div className="small-12 columns">
							...
							<div className={singleStyles.byline}>
								<span>by {data.author_name} on {data.display_date}</span>
							</div>
						</div>
					</div>
